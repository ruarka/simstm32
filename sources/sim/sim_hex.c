...
int read_ihex_chunks( const char * fname, ihex_chunk_p * chunks, uint32_t* sladdr)
{
	if (!fname || !chunks)
		return -1;
	...
	*chunks = NULL;
	...
	while (!feof(f)) {
		...
		if (!fgets(line, sizeof(line)-1, f))
			continue;
		if (line[0] != ':') {
			fprintf(stderr, "STM32: '%s' invalid ihex format (%.4s)\n", fname, line);
			break;
		}
		uint8_t bline[64];
		...
		switch (bline[3]) {
			...
			case 4:
				...
				continue;
			case 5:
//				*sladdr = ((bline[7] << 24) | bline[6]<<16 | bline[5]<<8 | bline[4] );
				*sladdr = ((bline[4] << 24) | bline[5]<<16 | bline[6]<<8 | bline[7] );
				continue;
			default:
...
uint8_t* read_ihex_file( const char * fname, uint32_t* dsize, uint32_t* start, uint32_t* start_linear_address )
{
	ihex_chunk_p chunks = NULL;
	int count = read_ihex_chunks(fname, &chunks, start_linear_address);
	uint8_t * res = NULL;
	...
	if (count > 1) {
		fprintf(stderr, "STM32: '%s' ihex contains more chunks than loaded (%d)\n",
				fname, count);
	}
